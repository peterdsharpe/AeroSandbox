...
if fuel_type == "LH2":
    fuel_tank_wall_thickness = (
        0.0612  # from Brewer, Hydrogen Aircraft Technology pg. 203
    )
    fuel_density = 70  # kg/m^3
    fuel_specific_energy = 119.93e6  # J/kg; lower heating value due to liquid start
    fuel_tank_fuel_mass_fraction = 1 / (
        1 + 0.356
    )  # from Brewer, Hydrogen Aircraft Technology pg. 203
elif fuel_type == "GH2":
    fuel_tank_wall_thickness = (
        0.0612  # from Brewer, Hydrogen Aircraft Technology pg. 203
    )
...
z_fuse_sections.append(
    linear_map(
        f_in=z_sect_nondim, min_in=0, max_in=1, min_out=0, max_out=fuselage_cabin_radius
    )
)
r_fuse_sections.append(
    linear_map(
        f_in=r_sect_nondim, min_in=0, max_in=1, min_out=0, max_out=fuselage_cabin_radius
    )
)
...
fuse = asb.Fuselage(
    ...
    xsecs=[
        asb.FuselageXSec(
            xyz_c=[x_fuse_sections[i], 0, z_fuse_sections[i]], radius=r_fuse_sections[i]
        )
...
wing_span = opti.variable(init_guess=214 * u.foot, lower_bound=0, freeze=True)
...
wing_x_le = opti.variable(
    init_guess=0.5 * x_fwd_tank_to_cabin
    + 0.5 * x_cabin_to_aft_tank
    - 0.5 * wing_root_chord,
    freeze=True,
)
...
wing = (
    asb.Wing(name="Main Wing", symmetric=True, xsecs=[wing_root, wing_yehudi, wing_tip])
...
hstab_span = opti.variable(init_guess=70.8 * u.foot, lower_bound=0, freeze=True)
hstab_half_span = hstab_span / 2

hstab_root_chord = opti.variable(init_guess=23 * u.foot, lower_bound=0, freeze=True)

hstab_LE_sweep_deg = opti.variable(init_guess=35, lower_bound=0, freeze=True)

elevator = asb.ControlSurface(
    name="Elevator",
    deflection=opti.variable(init_guess=-5, freeze=True),
    hinge_point=0.75,
)
...
vstab_span = opti.variable(init_guess=33 * u.foot, lower_bound=0, freeze=True)

vstab_root_chord = opti.variable(init_guess=25 * u.foot, lower_bound=0, freeze=True)

vstab_LE_sweep_deg = opti.variable(init_guess=35, lower_bound=0, freeze=True)

vstab_root = asb.WingXSec(
    xyz_le=[0, 0, 0], chord=vstab_root_chord, airfoil=vstab_airfoil
)
...
mass_props["payload_proportional_weights"] = (
...
K_ws = (
    ...
    * (wing_span / fuselage_structural_length * np.tand(wing.mean_sweep_angle()))
...
design_max_thrust_ratio_to_ref_engine = design_max_thrust_engine / ref_engine["thrust"]

engine_fan_diameter = (
    ref_engine["fan_diameter"] * design_max_thrust_ratio_to_ref_engine**0.5
)
engine_outer_diameter = (
    ref_engine["outer_diameter"] * design_max_thrust_ratio_to_ref_engine**0.5
)
x_engines = wing_x_le + wing_yehudi_x

mass_props["engines"] = asb.mass_properties_from_radius_of_gyration(
    mass=(n_engines * ref_engine["mass"] * design_max_thrust_ratio_to_ref_engine**1.1),
    x_cg=x_engines,
)

# Landing gear mass
main_landing_gear_length = np.softmax(
    1.1 * engine_outer_diameter, (fuse.length() / 2) * np.tand(3.5), hardness=10
)
...
mass_props["engine_controls"] = asb.mass_properties_from_radius_of_gyration(
    mass=(5 * n_engines + 0.80 * (x_cabin_midpoint / u.foot) * n_engines) * u.lbm,
    x_cg=(x_engines + x_nose) / 2,
)

mass_props["starter"] = asb.mass_properties_from_radius_of_gyration(
    mass=(49.19 * (mass_props["engines"].mass / u.lbm / 1000) ** 0.541) * u.lbm,
    x_cg=x_engines,
)

# Flight controls mass
control_surface_area = 0.15 * (wing.area() + hstab.area() + vstab.area())
control_surface_sizing_Iyy_aircraft = design_mass_TOGW * wing_to_hstab_distance**2
...
mass_props["instruments"] = asb.mass_properties_from_radius_of_gyration(
    mass=(
        ...
        * n_crew**0.541
        * n_engines
        * (fuselage_cabin_length / u.foot * wing_span / u.foot) ** 0.5
    )
    * u.lbm,
    x_cg=x_nose_to_fwd_tank,
)
...
mass_props["hydraulics"] = asb.mass_properties_from_radius_of_gyration(
    mass=(0.2673 * 6 * (fuselage_cabin_length / u.foot * wing_span / u.foot) ** 0.937)
    * u.lbm,
    x_cg=wing.xsecs[0].xyz_le[0] + wing.xsecs[0].chord,
)
...
mass_props["avionics"] = asb.mass_properties_from_radius_of_gyration(
    mass=(1.73 * (1100) ** 0.983) * u.lbm, x_cg=x_nose_to_fwd_tank
)

# Anti-ice mass
mass_props["anti-ice"] = asb.mass_properties_from_radius_of_gyration(
    mass=0.002 * design_mass_TOGW, x_cg=wing.aerodynamic_center()[0]
)

# Handling gear mass
mass_props["handling_gear"] = asb.mass_properties_from_radius_of_gyration(
    mass=3e-4 * design_mass_TOGW, x_cg=x_cabin_midpoint
)
...
fwd_fuel_tank_interior_volume = fuel_tank_xsec_area * (
    fwd_fuel_tank_length - 2 * fuel_tank_wall_thickness
)
aft_fuel_tank_interior_volume = fuel_tank_xsec_area * (
    aft_fuel_tank_length - 2 * fuel_tank_wall_thickness
)
...
mass_props_full_fuel_fwd = asb.mass_properties_from_radius_of_gyration(
    mass=fuel_density * fwd_fuel_tank_interior_volume, x_cg=x_fwd_tank_midpoint
)
mass_props_full_fuel_aft = asb.mass_properties_from_radius_of_gyration(
    mass=fuel_density * aft_fuel_tank_interior_volume, x_cg=x_aft_tank_midpoint
)

mass_props["fuel"] = mass_props_full_fuel_fwd + mass_props_full_fuel_aft

mass_props["tanks"] = (
    mass_props["fuel"] / fuel_tank_fuel_mass_fraction * fuel_tank_tank_mass_fraction
)
...
mass_props_empty_fuselage = mass_props_empty - (
    mass_props["wing"] + mass_props["hstab"] + mass_props["vstab"]
)
...
opti.subject_to([mass_props_TOGW.mass < design_mass_TOGW])
