...
df = pd.DataFrame(
    {
        ...
        / (43.02e6 * turboprops["SFC (TO) [lb/shp hr]"] * (u.lbm / u.hp / u.hour)),
    }
).dropna()
...
if __name__ == "__main__":
    ...
    x = np.geomspace(df["Weight [kg]"].min() / 3, df["Weight [kg]"].max() * 3, 100)
    y = np.blend(
        fit.parameters["a"]
        + (np.log10(x) - fit.parameters["wcen"]) / fit.parameters["wscl"],
        value_switch_high=1,
        value_switch_low=0,
    )
    plt.plot(
