...
def get_data(filename):
    ...
    data = {k: v[order][indices] for k, v in data.items()}
...
def find_mach_dd(machs, CDs):
    ...
    return optimize.minimize_scalar(
        fun=lambda M: (dCDdM(M) - 0.1) ** 2, method="bounded", bounds=(0.5, 0.9)
    ).x


machs = np.linspace(0, 1.3, 500)
af = asb.Airfoil("rae2822")
ab_aero = af.get_aero_from_neuralfoil(1, Re=6.5e6, mach=machs, model_size="large")
ab_aero["mach"] = machs
...
for y in ["CD"]:
    ...
    for analysis, data in datas.items():
        plt.plot(
            ...
            zorder=5
            if "NeuralFoil" in analysis
            else 4
            if "XFoil" in analysis
            else 3
            if "MSES" in analysis
            else 2
            if "SU2" in analysis
