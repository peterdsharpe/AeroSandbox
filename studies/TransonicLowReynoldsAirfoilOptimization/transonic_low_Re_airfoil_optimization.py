...
def get_aero(
    ...
    if include_360_deg_effects:
        ...
        CL_if_separated, CD_if_separated, CM_if_separated = (
            airfoil_coefficients_post_stall(
                airfoil=asb.Airfoil("naca0012"), alpha=alpha
            )
        )
        ...
        is_separated = (
            np.softmax(alpha - alpha_stall_positive, alpha_stall_negative - alpha) / 3
        )
...
initial_aero = get_aero(
    kulfan_parameters=initial_guess_parameters, alpha=0, Re=5e5, mach=0.9
)
# pprint(initial_aero)
...
opti.subject_to(kulfan_parameters["upper_weights"] > kulfan_parameters["lower_weights"])
...
af = asb.Airfoil(
    name="optimized", coordinates=get_kulfan_coordinates(**kulfan_parameters)
)
# af.draw()
pprint(aero)
