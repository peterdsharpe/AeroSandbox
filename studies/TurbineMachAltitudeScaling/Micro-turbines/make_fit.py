...
if __name__ == "__main__":
    ...
    cm = p.mpl.colormaps.get_cmap("crest")
    ...
    for mach in [0, 0.3, 0.6, 0.9]:
        ...
        op_point = asb.OperatingPoint(
            atmosphere=atmo, velocity=mach * atmo.speed_of_sound()
        )
        sea_level_atmo = asb.Atmosphere()
        stagnation_density_ratios = (op_point.total_pressure() / atmo.temperature()) / (
    ...
    p.labelLines(lines=ax.lines, fontsize=8, alpha=0.8)
    ...
    plt.colorbar(
        label="Mach Number [-]", mappable=plt.cm.ScalarMappable(norm=norm, cmap=cm)
    )
    ...
    Machs, Altitudes = np.meshgrid(np.linspace(0, 0.9), np.linspace(0, 40000 * u.foot))

    atmo = asb.Atmosphere(altitude=Altitudes.flatten())
    op_point = asb.OperatingPoint(
        atmosphere=atmo, velocity=Machs.flatten() * atmo.speed_of_sound()
    )
    sea_level_atmo = asb.Atmosphere()
    stagnation_density_ratio = (op_point.total_pressure() / atmo.temperature()) / (
        sea_level_atmo.pressure() / sea_level_atmo.temperature()
    )
    ...
    p.show_plot(
        "\nFreestream Stagnation Density Ratio", "Mach [-]", "Altitude [foot]", dpi=300
    )
