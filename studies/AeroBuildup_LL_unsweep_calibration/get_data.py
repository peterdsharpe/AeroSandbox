...
def get_xnps(
    ...
    airplane = asb.Airplane(
        wings=[wing], c_ref=root_chord, b_ref=wing.span(), s_ref=wing.area()
    )

    op_point = asb.OperatingPoint(velocity=10, alpha=alpha)

    xyz_ref = [-0.01 * root_chord, 0, 0]
...
if __name__ == "__main__":
    ...
    vlm_xnps, ab_xnps, wings = get_xnps(
        AR=Aspect_ratio, taper=Taper, sweep_deg=Sweep_deg, root_chord=root_chord
    )
    ...
    cm = p.mpl.colormaps.get_cmap("rainbow")
    clim = (0, 1)
    ...
    p.show_plot("", "Sweep", "$\Delta x_{\\rm np} / c_{\\rm ref}$ [m]")
