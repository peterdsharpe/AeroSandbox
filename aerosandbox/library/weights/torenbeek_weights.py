...
def mass_wing_simple(
    ...
    """
    Compute the mass of a wing of an aircraft, according to Torenbeek's "Synthesis of Subsonic
    Airplane Design".
    ...
    A more detailed version of the wing weight model is available in the `mass_wing()` function
    in this same module.

    Parameters
    ----------
    wing : asb.Wing
        The wing object. Should be an AeroSandbox Wing object.
    design_mass_TOGW : float
        The design takeoff gross weight of the entire aircraft [kg].
    ultimate_load_factor : float
        The ultimate load factor of the aircraft. 1.5x the limit load factor.
    suspended_mass : float
        The mass of the aircraft that is suspended from the wing [kg].
    main_gear_mounted_to_wing : bool
        Whether the main gear is mounted to the wing structure.

    Returns
    -------
    float
        The total mass of the wing [kg].
    """
...
def mass_wing_high_lift_devices(
    ...
    """
    Estimate the weight of the high-lift devices on an airplane wing.

    Uses Torenbeek's method, which is based on multiple factors like wing design and flap
    deflection.

    Parameters
    ----------
    wing : asb.Wing
        An instance of AeroSandbox's Wing class.
    max_airspeed_for_flaps : float
        The maximum airspeed at which the flaps can be deployed [m/s].
    flap_deflection_angle : float
        The angle to which the flaps can be deflected [deg]. Default value is 30 degrees.
    k_f1 : float
        Configuration factor 1, with values:
    ...
    k_f2 : float
        Configuration factor 2, with values:
    ...
    -------
    float
        Mass of the wing's high-lift system only [kg].
    """
...
def mass_wing_basic_structure(
    ...
    """
    Compute the mass of the basic structure of the wing of an aircraft, according to
    ...
    ultimate_load_factor : float
        The ultimate load factor of the aircraft [-]. 1.5x the limit load factor.
    suspended_mass : float
        The mass of the aircraft that is suspended from the wing [kg]. It should exclude any
        wing attachments that are not part of the wing structure.
    never_exceed_airspeed : float
        The never-exceed airspeed of the aircraft [m/s]. Used for flutter calculations.
    main_gear_mounted_to_wing : bool
        Whether the main gear is mounted to the wing structure. Boolean.
    strut_y_location : float | None
        The spanwise-location of the strut (if any), as measured from the wing root [meters].
        If None, it is assumed that there is no strut (i.e., the wing is a cantilever beam).
    k_e : float
        Represents weight knockdowns due to bending moment relief from engines mounted in front
        of the elastic axis. See Torenbeek unlabeled equations, between C-3 and C-4.
    ...
    return_dict : bool
        Whether to return a dictionary of all the intermediate values, or just the final mass.
        Defaults to False, which returns just the final mass [kg].

    Returns
    -------
    float | dict[str, float]
        If return_dict is False (default), returns a single value: the mass of the basic
        wing [kg]. If return_dict is True, returns a dictionary of all the intermediate values.
    """
...
def mass_wing_spoilers_and_speedbrakes(wing: asb.Wing, mass_basic_wing: float) -> float:
    """
    Estimate the weight of the spoilers and speedbrakes according to Torenbeek's "Synthesis of
    Subsonic Airplane Design", 1976, Appendix C: "Prediction of Wing Structural Weight".

    N.B. the weight is coming out unrealistic and approx. 20-30% of the weight of the wing.
    This needs a correction. It uses normally the 12.2 kg/m^2 wing area.
    ...
    mass_basic_wing : float
        The basic weight of the wing (without spoilers, speedbrakes, flaps, slats) [kg].

    Returns
    -------
    float
        The mass of the spoilers and speed brakes only [kg].
    """
...
def mass_wing(
    ...
    """
    Compute the mass of a wing of an aircraft, according to Torenbeek's "Synthesis of Subsonic
    Airplane Design", 1976, Appendix C: "Prediction of Wing Structural Weight".
    ...
    never_exceed_airspeed : float
        The never-exceed airspeed of the aircraft [m/s]. Used for flutter calculations.
    max_airspeed_for_flaps : float
        The maximum airspeed at which the flaps are allowed to be deployed [m/s]. In the
        absence of other information, 1.8x stall speed is a good guess.
    main_gear_mounted_to_wing : bool
        Whether the main gear is mounted to the wing structure.
    flap_deflection_angle : float
        The maximum deflection angle of the flaps [deg].
    strut_y_location : float | None
        The y-location of the strut (if any), relative to the wing's leading edge [m]. If None,
        it is assumed that there is no strut (i.e., the wing is a cantilever beam).
    return_dict : bool
        Whether to return a dictionary of all the intermediate values, or just the final mass.
        Defaults to False, which returns just the final mass.

    Returns
    -------
    float | dict[str, float]
        If return_dict is False (default), returns a single value: the total mass of the
        wing [kg]. If return_dict is True, returns a dictionary of all the intermediate values.
    """
...
def mass_fuselage_simple(
    ...
    """
    Compute the mass of the fuselage, using Torenbeek's simple version of the calculation.
    ...
    ----------
    fuselage : asb.Fuselage
        The fuselage object. Should be an AeroSandbox Fuselage object.
    never_exceed_airspeed : float
        The never-exceed airspeed of the aircraft, in m/s.
    wing_to_tail_distance : float
        The distance from the quarter-chord of the wing to the quarter-chord of the tail,
        in meters.

    Returns
    -------
    float
        The mass of the fuselage, in kg.
    """
...
def mass_fuselage(
    ...
    """
    Compute the mass of the fuselage, using Torenbeek's detailed method (Appendix D).
    ...
    raise NotImplementedError(
        "This function is incomplete - S_g, W_str, W_fr, and k_lambda calculation are not yet defined."
    )
...
def mass_propeller(
    ...
    """
    Compute the mass of a propeller.
