...
altitudes_world = np.load(
    root / "datasets" / "winds_and_tropopause_global" / "altitudes.npy"
)
latitudes_world = np.load(
    root / "datasets" / "winds_and_tropopause_global" / "latitudes.npy"
)
day_of_year_world_boundaries = np.linspace(0, 365, 13)
day_of_year_world = (
    day_of_year_world_boundaries[1:] + day_of_year_world_boundaries[:-1]
) / 2
winds_95_world = np.load(
    root
    / "datasets"
    / "winds_and_tropopause_global"
    / "winds_95_vs_altitude_latitude_day.npy"
)
...
def wind_speed_world_95(altitude, latitude, day_of_year):
    ...
    return winds_95_world_model(
        {"altitude": altitude, "latitude": latitude, "day of year": day_of_year}
    )
...
day_of_year_trop = (
    day_of_year_trop_boundaries[1:] + day_of_year_trop_boundaries[:-1]
) / 2
...
tropopause_altitude_model = InterpolatedModel(
    x_data_coordinates={"latitude": latitudes_trop, "day of year": day_of_year_trop},
...
if __name__ == "__main__":
    ...
    def plot_tropopause_altitude():
        ...
        trop_alt = tropopause_altitude(
            Latitudes.flatten(), Day_of_years.flatten()
        ).reshape(Latitudes.shape)
