...
class TransverseGustPitchControl(ImplicitAnalysis):
    ...
    def __init__(
        self, reduced_time: np.ndarray, gust_profile: np.ndarray, velocity: float
    ):
    ...
    def _setup_unknowns(self):
        ...
        self.lift_coefficients = self.opti.variable(
            init_guess=1, n_vars=self.timesteps - 1
        )
    ...
    def _enforce_governing_equations(self):
        ...
        for i in range(self.timesteps - 1):
            ...
            self.lift_coefficients[i] += (
                2 * np.pi / self.velocity * (init_term[i] + integral_term)
            )

        # Calculate unsteady lift due to added mass
        self.lift_coefficients += (
            np.pi / 2 * np.cos(self.angles_of_attack[:-1]) ** 2 * da_ds
        )
    ...
    def calculate_transients(self):
        ...
        ds = self.reduced_time[1:] - self.reduced_time[:-1]
        da_ds = (
            self.optimal_pitching_profile_rad[1:]
            - self.optimal_pitching_profile_rad[:-1]
        ) / ds
        ...
        # Calculate unsteady lift due to added mass
        self.added_mass_lift = (
            np.pi / 2 * np.cos(self.optimal_pitching_profile_rad[:-1]) ** 2 * da_ds
        )
...
if __name__ == "__main__":
    ...
    ax2 = ax1.twinx()
    ax1.plot(
        reduced_time[:-1], optimal.optimal_lift_history, label="Total Lift", lw=2, c="k"
    )
    ...
    ax2.plot(
        reduced_time,
        optimal.optimal_pitching_profile_deg,
