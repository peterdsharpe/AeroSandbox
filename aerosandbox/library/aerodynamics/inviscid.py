...
def induced_drag(
    ...
    """
    Compute the induced drag associated with a lifting planar wing.
...
def oswalds_efficiency(
    ...
    """
    Compute the Oswald's efficiency factor for a planar, tapered, swept wing.
    ...
    ----------
    taper_ratio : float
        Taper ratio of the wing (tip_chord / root_chord) [-]
    aspect_ratio : float
        Aspect ratio of the wing (b^2 / S) [-]
    sweep : float
        Wing quarter-chord sweep angle [deg]
    fuselage_diameter_to_span_ratio : float
        Ratio of the fuselage diameter to the wingspan; used for a fuselage-interference
        correction factor [-]
    method : Literal["nita_scholz", "kroo"]
        The estimation method to use. One of "nita_scholz" (default) or "kroo".
...
def optimal_taper_ratio(
    ...
    """
    Compute the optimal (minimum-induced-drag) taper ratio for a given quarter-chord sweep angle.
...
def CL_over_Cl(
    ...
    """
    Compute the ratio of 3D lift coefficient (with compressibility) to the 2D lift coefficient.
    ...
    sweep : float
        The sweep of the wing, in degrees. To be most accurate, this should be the sweep at the
        locus of thickest points along the wing.
    Cl_is_compressible : bool
        This flag indicates whether the 2D airfoil data already has compressibility effects
        modeled.

        For example:

        * If this flag is True, this function returns: CL_3D / CL_2D, where CL_2D is the
          sectional lift coefficient based on the local profile at the freestream mach number.

        * If this flag is False, this function returns: CL_3D / CL_2D_at_mach_zero, where
          CL_2D_... is the sectional lift coefficient based on the local profile at mach zero.

        For most accurate results, set this flag to True, and then model profile
        characteristics separately.

    Returns
    -------
    float
        The ratio CL_3D / CL_2D.
    """
...
def induced_drag_ratio_from_ground_effect(
    ...
    """
    Compute the ratio of actual to free-flight induced drag experienced by a wing in ground effect.
    ...
    ----------
    h_over_b : float
        (Height above ground) divided by (wingspan).
    ...
    References
    ----------
    W. F. Phillips, D. F. Hunsaker, "Lifting-Line Predictions for Induced Drag and Lift in
    Ground Effect". Using Equation 5 from the paper, which is modified from a model from
    Torenbeek: Torenbeek, E. "Ground Effects", 1982.
    """
