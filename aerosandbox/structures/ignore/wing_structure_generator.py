...
class WingStructureGenerator:
    def __init__(
        ...
        self.oml = asb.Airplane(wings=[wing]).generate_cadquery_geometry(
    ...
    def add_ribs_from_section_span_fractions(
        ...
        for s in section_span_fractions:
            af = xsec_a.airfoil.blend_with_another_airfoil(
                airfoil=xsec_b.airfoil, blend_fraction=s
            )

            chord = (1 - s) * xsec_a.chord + s * xsec_b.chord
            ...
            self.ribs[span] = (
                cq.Workplane(
                    inPlane=cq.Plane(
                        origin=tuple((1 - s) * xsec_a.xyz_le + s * xsec_b.xyz_le),
                        xDir=tuple(csys[0]),
                        normal=tuple(-csys[1]),
                    )
                )
                .spline(listOfXYTuple=[tuple(xy * chord) for xy in af.coordinates])
                .close()
                .extrude(rib_thickness / 2, combine=False, both=True)
            )
    ...
    def add_ribs_from_xsecs(
        ...
        for i in indexes:
            ...
            self.ribs[span] = (
                ...
                .spline(listOfXYTuple=[tuple(xy * xsec.chord) for xy in af.coordinates])
                .close()
                .extrude(rib_thickness / 2, combine=False, both=True)
            )
    ...
    def add_tube_spar(
        ...
        section_index = (
            np.argwhere(self._cumulative_spans_up_to_section > span_location_root)[0][0]
            - 1
        )

        section_span_fraction = (
            span_location_root - self._cumulative_spans_up_to_section[section_index]
        ) / self._sectional_spans[section_index]

        root_csys = self.wing._compute_frame_of_section(section_index)

        root_le_point = (1 - section_span_fraction) * wing.xsecs[
            section_index
        ].xyz_le + section_span_fraction * wing.xsecs[section_index + 1].xyz_le
        root_chord = (1 - section_span_fraction) * wing.xsecs[
            section_index
        ].chord + section_span_fraction * wing.xsecs[section_index + 1].chord
        root_point = (
            root_le_point
        ...
        section_index = (
            np.argwhere(self._cumulative_spans_up_to_section > span_location_tip)[0][0]
            - 1
        )

        section_span_fraction = (
            span_location_tip - self._cumulative_spans_up_to_section[section_index]
        ) / self._sectional_spans[section_index]

        tip_csys = self.wing._compute_frame_of_section(section_index)

        tip_le_point = (1 - section_span_fraction) * wing.xsecs[
            section_index
        ].xyz_le + section_span_fraction * wing.xsecs[section_index + 1].xyz_le
        tip_chord = (1 - section_span_fraction) * wing.xsecs[
            section_index
        ].chord + section_span_fraction * wing.xsecs[section_index + 1].chord
        tip_point = (
            tip_le_point
        ...
        root_plane = cq.Plane(
            origin=tuple(root_point), xDir=tuple(root_csys[0]), normal=tuple(normal)
        )

        tip_plane = cq.Plane(
            origin=tuple(tip_point), xDir=tuple(tip_csys[0]), normal=tuple(normal)
        )

        ### Make the spar
        root_wire = cq.Workplane(inPlane=root_plane).circle(
            radius=2 * diameter_root / 2
        )
        tip_wire = cq.Workplane(inPlane=tip_plane).circle(radius=2 * diameter_tip / 2)
...
wing = asb.Wing(
    ...
    symmetric=True,
    xsecs=[
        asb.WingXSec(chord=0.2, airfoil=af),
        asb.WingXSec(xyz_le=[0.05, 0.5, 0], chord=0.15, airfoil=af),
        asb.WingXSec(xyz_le=[0.1, 0.8, 0.1], chord=0.1, airfoil=af),
    ],
)

s = WingStructureGenerator(wing, default_rib_thickness=1 / 16 * u.inch)
