...
def get_trajectory(
    ...
    if sideforce:
        ...
        dyn.add_force(
            Fy=q * 1 * (0.1) ** 2 * np.sin(2 * np.pi * frequency * time), axes="wind"
        )
...
def test_final_position_Cartesian_with_sideforce():
    dyn = get_trajectory(
        parameterization=asb.DynamicsPointMass3DCartesian, drag=True, sideforce=True
    )
...
def test_final_position_Cartesian_with_drag():
    dyn = get_trajectory(
        parameterization=asb.DynamicsPointMass3DCartesian, drag=True, sideforce=False
    )
...
def test_final_position_Cartesian_no_drag():
    dyn = get_trajectory(
        parameterization=asb.DynamicsPointMass3DCartesian, drag=False, sideforce=False
    )
...
def test_cross_compare_with_drag():
    dyn1 = get_trajectory(parameterization=asb.DynamicsPointMass3DCartesian, drag=True)
    dyn2 = get_trajectory(
        parameterization=asb.DynamicsPointMass3DSpeedGammaTrack, drag=True
    )
...
def test_cross_compare_no_drag():
    dyn1 = get_trajectory(parameterization=asb.DynamicsPointMass3DCartesian, drag=False)
    dyn2 = get_trajectory(
        parameterization=asb.DynamicsPointMass3DSpeedGammaTrack, drag=False
    )
    assert dyn1[-1].x_e == pytest.approx(dyn2[-1].x_e, abs=1e-2, rel=1e-2)
    assert dyn1[-1].y_e == pytest.approx(dyn2[-1].y_e, abs=1e-2, rel=1e-2)
    assert dyn1[-1].z_e == pytest.approx(dyn2[-1].z_e, abs=1e-2, rel=1e-2)
...
if __name__ == "__main__":
    dyn = get_trajectory(asb.DynamicsPointMass3DSpeedGammaTrack, plot=True)
    pytest.main()
