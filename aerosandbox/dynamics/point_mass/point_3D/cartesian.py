...
class DynamicsPointMass3DCartesian(_DynamicsPointMassBaseClass):
    ...
    * with velocity parameterized in Cartesian coordinates
    ...
    def gamma(self):
        ...
        return np.arctan2(
            -self.w_e,
            (self.u_e**2 + self.v_e**2 + 1e-200)  # To avoid gradient singularities
            ** 0.5,
        )
    ...
    def convert_axes(
        ...
        if from_axes == "earth":
            ...
            x_e = (
                ...
                + rot_w_to_e[0][1] * y_from
                + rot_w_to_e[0][2] * z_from
            )
            y_e = (
                rot_w_to_e[1][0] * x_from
                + rot_w_to_e[1][1] * y_from
                + rot_w_to_e[1][2] * z_from
            )
            z_e = (
                rot_w_to_e[2][0] * x_from
                + rot_w_to_e[2][1] * y_from
        ...
        if to_axes == "earth":
            ...
            x_to = (
                rot_w_to_e[0][0] * x_e + rot_w_to_e[1][0] * y_e + rot_w_to_e[2][0] * z_e
            )
            y_to = (
                rot_w_to_e[0][1] * x_e + rot_w_to_e[1][1] * y_e + rot_w_to_e[2][1] * z_e
            )
            z_to = (
                rot_w_to_e[0][2] * x_e + rot_w_to_e[1][2] * y_e + rot_w_to_e[2][2] * z_e
            )
        else:
            x_w = (
                rot_w_to_e[0][0] * x_e + rot_w_to_e[1][0] * y_e + rot_w_to_e[2][0] * z_e
