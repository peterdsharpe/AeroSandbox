...
class panel1(AeroProblem):
    ...
    def make_panels(self):
        ...
        for wing_num in range(len(self.airplane.wings)):
            ...
            nondim_xsec_coordinates = np.empty(
                (2 * n_chordwise_coordinates - 1, 0, 2))  # MxNx2 array of airfoil coordinates.
            ...
            xsec_coordinates = (xsec_xyz_le +
                                xsec_local_back * np.expand_dims(xsec_chord, axis=2) * np.expand_dims(
                        nondim_xsec_coordinates[:, :, 0], 2) +
                                xsec_local_up * np.expand_dims(xsec_chord * xsec_scaling_factor,
                                                               axis=2) * np.expand_dims(
                        nondim_xsec_coordinates[:, :, 1], 2)
                                )

            # -----------------------------------------------------
            # Interpolate the coordinates between xsecs
            wing_coordinates = np.empty(
                (2 * n_chordwise_coordinates - 1, 0, 3))  # MxNx3 of all coordinates on the wing.
            # First index is chordwise point #, second index is spanwise point #, third is xyz.
