...
def _calculate_induced_velocity_line_singularity_panel_coordinates(
    ...
    if skip_vortex_math:
        ...
        u_vortex_term_2_quantity = (gamma_start * xp_panel_end + d_gamma * xp_field) / (
            tau * xp_panel_end
        )
        ...
        v_vortex_term_2 = np.where(
            is_on_panel,
            d_gamma / tau,
            u_vortex_term_1_quantity * (xp_panel_end / yp_field_regularized - d_theta),
        )
    ...
    if skip_source_math:
        ...
        v_source_term_2_quantity = (sigma_start * xp_panel_end + d_sigma * xp_field) / (
            tau * xp_panel_end
        )
        # Calculate v_source
        ...
        u_source_term_2 = np.where(
            is_on_panel,
            -d_sigma / tau,
            -v_source_term_1_quantity * (xp_panel_end / yp_field_regularized - d_theta),
        )
...
def _calculate_induced_velocity_line_singularity(
    ...
    xp_field = (
        x_field_relative * xp_hat_x + y_field_relative * xp_hat_y
    )  # dot product with the xp unit vector
    yp_field = (
        x_field_relative * yp_hat_x + y_field_relative * yp_hat_y
    )  # dot product with the xp unit vector
