...
def plot_with_bootstrapped_uncertainty(
    ...
    if not (np.all(ci > 0) and np.all(ci < 1)):
        raise ValueError(
            "Confidence interval values in `ci` should all be in the range of (0, 1)."
        )
    ...
    if len(ci) != 0:
        ...
        for i, ci_val in enumerate(ci):
            settings = dict(
                color=color, alpha=ci_to_alpha_mapping(ci_val), linewidth=0, zorder=1.5
            )
            plt.fill_between(
                x_fit,
                lower_ci[i],
            ...
            plt.fill_between(x_fit, upper_ci[i], upper_ci[i + 1], **settings)
...
if __name__ == "__main__":
    ...
    p.show_plot("Spline Bootstrapping Test", r"$x$", r"$y$", legend=False)
