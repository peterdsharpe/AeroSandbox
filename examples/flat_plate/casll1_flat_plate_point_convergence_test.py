...
for n in ns:
    ...
    CLs.append(ap_sol.CL)
    CDs.append(ap_sol.CD)

CLs = np.array(CLs)
CDs = np.array(CDs)

CL_errors = np.abs(CLs-CLs[-1])
CD_errors = np.abs(CDs-CDs[-1])

import matplotlib.pyplot as plt

plt.figure()
plt.subplot(2,1,1)
plt.loglog(ns[:-1], CL_errors[:-1], '.-')
plt.xlabel("Number of Panels")
plt.ylabel("CL Error")
plt.title(airplane.wings[0].xsecs[0].spanwise_spacing)
plt.grid(True)
plt.subplot(2,1,2)
plt.loglog(ns[:-1], CD_errors[:-1], '.-')
plt.xlabel("Number of Panels")
plt.ylabel("CD Error")
plt.grid(True)
plt.show()