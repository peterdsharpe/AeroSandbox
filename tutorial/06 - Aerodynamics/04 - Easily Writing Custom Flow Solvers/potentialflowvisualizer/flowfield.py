...
class Flowfield:
    ...
    def draw(
        ...
        p.contour(
            ...
            contour_kwargs={"linestyles": "solid", "alpha": 0.4},
        )

        if set_equal:
            p.equal()

        p.show_plot(f"Potential Flow: {scalar_to_plot}", "$x$", "$y$", show=show)
    ...
    def draw_streamlines(
        ...
        X, Y = np.meshgrid(x_points, y_points)
        U = np.reshape(
            self.get_x_velocity_at(np.stack([X.flatten(), Y.flatten()], axis=1)),
            X.shape,
        )
        V = np.reshape(
            self.get_y_velocity_at(np.stack([X.flatten(), Y.flatten()], axis=1)),
            X.shape,
        )
        ...
        if norm is None:
            ...
            norm = LogNorm(np.quantile(velmag, 0.05), np.quantile(velmag, 0.95))
        ...
        p.show_plot("Potential Flow: Streamlines", "$x$", "$y$", show=show)
